...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/content/upload", isAuthenticated, upload.single('file'), async (req: any, res) => {
    try {
      ...
      try {
        if (req.body.searchUsernames) {
          searchUsernames = typeof req.body.searchUsernames === 'string' 
            ? JSON.parse(req.body.searchUsernames)
            : req.body.searchUsernames;
        }
      } catch (error) {
        console.log('[UPLOAD] Failed to parse searchUsernames:', error);
      }
      
      // Use the already parsed searchUsernames
      const parsedUsernames = searchUsernames.filter((u: string) => u && u.trim());
